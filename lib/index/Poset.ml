...
module Poset = struct
    ...
    let init env = let g = G.create() in 
        ...
        G.add_vertex g node; {env; graph = g; lowest = node}
    ...
    let add {env ; graph; lowest} elt_0 = 
        ...
        let rec insert_edges l = match l with
            | [] -> ()
            | (Replace edge)::rest -> 
        ...
        let get_couples edge = 
            ...
            in aux [] (G.succ graph (G.E.dst edge))
        in 
        ...
        let rec visit already_seen edge = 
            ...
            | Smaller -> let push elt = Queue.push elt to_visit in
                ...
                let new_edges = 
                    ...
                    | _ -> List.iter push l; []
                in new_edges @ visit_next already_seen
            | Uncomparable -> let push elt = Queue.push elt to_visit in
                let l = get_couples edge in 
                List.iter push l; 
                visit_next already_seen
        
        and visit_next already_seen = match Queue.take_opt to_visit with
            | None -> []
            | Some edge ->
              if Edge_set.mem edge already_seen then 
                visit_next already_seen
              else
    ...
    let iter_succ t elt f = 
        let rec aux l = match l with
            |[] -> ()
            |h::q -> f h ; aux (G.succ t h); aux q
        in aux [elt];;

    let iter_pred t elt f = 
        let rec aux l = match l with
            |[] -> ()
            |h::q -> f h ; aux (G.pred t h); aux q
        in aux [elt];;
