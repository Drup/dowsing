...
module System : sig
  ...
  (* Here the cut function is used in case we know that a type will not be compatible
     ...
     in the associated diophantine system. This is currently used for the non_arrow variable
     in the arrow system. *)
  let add_problem cut get_index nb_atom {ACTerm. left; right} =
    ...
    let add cut dir r =
      if not (cut r) then
        ...
        | None -> ()
    in
    Array.iter (add cut 1) left ;
    Array.iter (add cut (-1)) right ;
    equation
  ...
  let make problems : t * t List.t =
    ...
    let var_system =
      ...
      let nb_non_var = 0 in
      let system =
        List.map (add_problem (fun _ -> false) (get_index vars) nb_atom) problems
        |> Array.of_list
      in
    ...
    let gen_shape_system nb_frees types_map =
      ...
      let system =
        List.map (add_problem (fun _ -> false)
    ...
    let gen_arrow_system nb_frees types_map =
      ...
      let nb_non_var = nb_frees in
      let system =
        List.map (add_problem Type.is_non_arrow_var
          (get_index_shape vars types_map nb_frees) nb_atom) problems
        |> Array.of_list
      in
      { nb_atom ; assoc_type ; nb_non_var; nb_non_tuple ; system }
    in
    var_system,
    if nb_arrow > 0 then (gen_arrow_system nb_arrow arrow::shape_systems)
    else shape_systems
