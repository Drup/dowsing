...
\DeclareMathOperator*{\msetNbigplus}{\scalerel*{+}{\sum}^\#_\N}
\DeclareMathOperator*{\msetGbigplus}{\scalerel*{+}{\sum}^\#_\G}
...
% Nous exposons maintenant comment \emph{transformer} un problème équationnel\laure{pour quoi faire}. Dans cette présentation, six règles suffisent, \laure{plus formellement décrites dans la définition~\ref{def:transfo}}
% >>>>>>> 1b38a4b (fin section 2)
\begin{itemize}
...
% =======
% Cet algorithme est correct et complet en ce sens : deux types $\tau_1$ et $\tau_2$ admettent un unificateur $\sigma$ minimal\laure{notion non expliquée} pour $\leqslant_\Sigma$ \ssi $\mathscr{A} (\tau_1, \tau_2)$ retourne $\sigma$. %Nous ne le redémontrons pas ici.
% >>>>>>> 1b38a4b (fin section 2)
...
Le matching et l'unification modulo $\E$ sont décidables \cite{Narendran_Pfenning_Statman}. Gabriel Radanne a implémenté un algorithme d'unification, qui servira de base à notre implementation, dont nous expliquons les principes ici.

% . Néanmoins\laure{pourquoi néanmoins}, comme nous l'avons vu auparavant, le matching s'y réduit. L'usage indique d'ailleurs qu'il n'est généralement pas souhaitable d'instancier le type demandé ; c'est pourquoi nous en avons fait le comportement par défaut, avec la possibilité d'expliciter les variables de types instanciables. \laure{Je ne comprends pas le paragraphe commençant à ``l'usage'', et à mon avis c'est inutile}
\medskip
L'algorithme d'unification modulo $\E$ est conçu à partir d'un algorithme d'unification modulo associativité et commutativité (AC)~\cite{Boudet}. Ce dernier possède les mêmes traits que l'algorithme d'unification syntaxique présenté à la Section~\ref{sec:unif_syntax}: il s'agit de transformer un problème à l'aide d'un jeu de règles. Les objets qu'il manipule s'avèrent toutefois plus élaborés. Le plus important est l'ajout d'une règle de transformation résolvant un \emph{problème AC élémentaire} n'impliquant que des termes simples (variables et constantes) et un unique opérateur AC.

\medskip
L'algorithme de Boudet a été adapté de la manière suivante :
\begin{itemize}[nosep]
%  \item la représentation des termes à base de pointeurs\laure{Je pense qu'on se fiche de ce détail} se traduit par d'autres mécanismes d'indirection en OCaml ;
...
\laure{Pour ....., nous devons normaliser les types}
%Attachons-nous à formaliser la \emph{normalisation des types}. Pour se faire,

Nous définissons d'abord les \emph{types normalisés} — c'est-à-dire le résultat de la normalisation — puis la fonction de normalisation elle-même.

\smallskip
Étant donné un ensemble $A$, on note $A^\#$ l'ensemble des multi-ensembles finis sur $A$ et $\cdot +^\#_A \cdot$, $| \cdot |^\#_A$, $\cdot =^\#_A \cdot$ et $\cdot \subseteq^\#_A \cdot$ respectivement la somme, le cardinal, l'égalité et l'inclusion sur $A^\#$. Nous n'explicitons pas la représentation. On suppose toutefois qu'il est licite d'écrire $\N^\# \subseteq \N$ dans la définition des \emph{types normalisés}. En Coq, on utiliserait typiquement des listes ou vecteurs.\laure{je pense qu'on s'en fiche}

%Définissons donc les \emph{types normalisés}.

Un \emph{type normalisé} est soit :
\begin{itemize}[nosep]
  \item une variable ;
...
\laure{le plus important est de dire ce qu'il y a de différent avec la def d'un type 10 pages plus tôt}
...
\begin{exemple}
  ...
  \begin{itemize}
    ...
    \end{itemize}
    \laure{et un type pas normalisé}
\end{exemple}

Afin de désigner la forme d'un type normalisé de manière concise, introduisons par ailleurs la notion de \emph{genre}. \laure{NO WAY, utiliser un autre terme.}
...
\begin{definition}[type normalisé bien formé]
  ...
  \begin{mathpar}
    ...
    \inferrule*
      ...
      {f (\nu_1, \dots, \nu_{|f|_\F})\ \bold{pbf}}
    \end{mathpar}
    \laure{Souvent on définit plutôt un prédicat par $\models_{BF}\nu$}
  On note $\N^{\bold{bf}}$ l'ensemble des types normalisés bien formés et $\N^{\bold{pbf}}$ l'ensemble des types presque bien formés.
\end{definition}

\begin{exemple}
  \laure{bla est bf alors que blu est pbf et foo n'est ni bf ni pbf}
\end{exemple}
...
\begin{definition}[forme normalisée d'un type]
  La forme normalisée d'un type $\tau$, notée $\norm (\tau)$, est définie inductivement par :
  \begin{align*}
      \norm (\alpha) &=
      \alpha
    \\
    ...
    \\
      ...
      \nu^\#
  \end{align*}
  \laure{attention ça dépasse de la marge à droite}
\end{definition}
...
\begin{theoreme} \label{norm_bf}
  La forme normalisée de tout type est bien formée.
\end{theoreme}

\begin{preuve}
  La preuve se fait par  induction structurelle et par cas sur le type.
  \begin{itemize}[label={\textbf{Cas}}]
    \item
      $\alpha$ : trivial. \laure{immédiat, pas trivial}
    \item
      $\unit$ : trivial.
    \item
      $\tau_1 \times \tau_2$ : \\
...
\laure{je te laisse faire un query replace de trivial}

Nous savons comment passer des types aux types types normalisés. Reste à lier des deux domaines\laure{quels deux domaines}. Ce à quoi nous voulons arriver est un théorème d'équivalence pour l'unification. Il s'agit donc de définir la notion d'unifiabilité pour les types normalisés. De la même manière qu'auparavant, voyons tout d'abord par la notion de \emph{substitution} :

\laure{à quoi sert de tout redonner !}
...
\end{definition}
  ...
  \laure{big plus undefined} 

\begin{exemple}
  Nous donnons ci-dessous quelques exemples d'applications de substitutions :
  \begin{itemize}
...
\begin{theoreme} 
\label{bf-implique-subst-bf} %pas de underscore dans les label, ça plante la coloration de mon editeur
  Pour tout $\nu$ dans $\N^{\bold{bf}}$ et $\theta$ dans $\Theta^{\bold{bf}}$, $\hat\theta (\nu)$ est bien formé.
\end{theoreme}

\begin{preuve}
  La preuve est faite par induction structurelle sur $\nu$.
  \begin{itemize}[label={\textbf{Cas}}]
    \item
...
Finalement, nous obtenons une notion d'unification pour les types normalisés :
...
Le théorème fondamental \laure{lequel !ci-dessous ? } assure alors l'équivalence entre unifiabilité sur les types et unifiabilité sur les types normalisés. La démonstration n'étant pas complète \laure{au sens terminée ?}, nous l'admettons.

\begin{theoreme}
  Deux types admettent un unificateur de type bien formé \ssi leurs formes normalisées admettent un unificateur de types normalisés bien formé.
\end{theoreme}

\laure{C'est très dommage, tu passes un temps fou à redéfinir des trucs alors que c'est la preuve importante}


\laure{En conclusion, pour ... il suffit de résoudre .... ce qui est facilité par ...}
...
\begin{preuve}
  ...
  \[ \mu_g (\hat\theta (\nu^\# \rightarrow \nu)) = \mu_g' \left( \msetNbigplus_{i \in \interval 1 n} \norm' (\hat\theta (\nu_i)) \right) \]
...
\begin{definition}[$\mathrm{crit}^2$]
  ...
  \[ \mathrm{encode}^2 (\nu) = \left( \mathrm{vars\mathhyphen echine} (\nu), \msetGbigplus_{g \notin \{ \bold{var}, \bold{uplet} \}} \msetGbigplus_{\interval 1 {\mu_g (\nu)}} \mset{g} \right) \]
  \begin{align*}
