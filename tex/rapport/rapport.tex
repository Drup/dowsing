...
\DeclareMathOperator*{\bigtimes}{\scalerel*{\times}{\prod}}
...
Etant donné un ensemble $A$, on note $A^\#$ l'ensemble des multi-ensembles finis sur $A$ et $\cdot +^\#_A \cdot$, $| \cdot |^\#_A$ et $\cdot \subseteq^\#_A \cdot$ respectivement la somme, le cardinal et l'inclusion sur $A^\#$. Nous n'explicitons pas la représentation. On suppose toutefois qu'il est licite d'écrire $\N^\# \subseteq \N$ dans la définition des \emph{types normalisés}. En Coq, on utiliserait typiquement des listes ou vecteurs.
...
\begin{theoreme} \label{unifT_equiv_unifN}
...
\subsection{Bonne formation d'une normalisée}
...
\subsection{Préservation de la bonne formation par application d'une substitution bien formée}
...
\begin{preuve}
  ...
  \begin{itemize}[label={\textbf{Cas}}]
    \item
      ...
      Absurde car $\theta$ est bien formée.
    \item
      $\nu = \mset{\nu_1, \dots, \nu_n}$ : \\
...
\subsection{Equivalence des unifiabilités}

\begin{theoreme} \label{unifT_equiv_unifN}
  Deux types admettent un unificateur bien formé \ssi leurs normalisées admettent un unificateur bien formé.
\end{theoreme}

\begin{definition}[normalisée d'une substitution de types]
	La normalisée d'une substitution de types $\sigma$, notée $\norm (\sigma)$, est la substitution de types normalisés définie par :
	\[ \forall \alpha \in \V,\ (\norm (\sigma)) (\alpha) = \norm (\sigma (\alpha)) \]
\end{definition}

\begin{definition}[normalisée inverse d'un type normalisé]
	La normalisée inverse d'un type normalisé $\nu$, notée $\inorm (\nu)$, est définie inductivement par :
	\begin{align*}
			\inorm (\alpha) &=
			\alpha
		\\
...
\begin{definition}[normalisée inverse d'une substitution de types normalisés]
	La normalisée inverse d'une substitution de types normalisés $\theta$, notée $\inorm (\theta)$, est la substitution de types définie par :
	\[ \forall \alpha,\ (\inorm (\theta)) (\alpha) = \inorm (\theta (\alpha)) \]
\end{definition}

\begin{preuve}
	\begin{itemize}[topsep=0pt]
		\item[$(\Rightarrow)$]
			Soient deux types $\tau_1$ et $\tau_2$ admettant un unificateur bien formé $\sigma$ :
			\[ \hat\sigma (\tau_1) \equiv_\T \hat\sigma (\tau_2) \]
			Par le lemme \ref{equivT_equiv_egaliteN}, on peut passer aux normalisées :
			\[ \norm (\hat\sigma (\tau_1)) = \norm (\hat\sigma (\tau_2)) \]
			Par le lemme \ref{norm_subst_eq_norm_subst_norm}, il vient que $\norm (\sigma)$ est un unificateur des normalisées de $\tau_1$ et $\tau_2$ :
			\[ (\widehat\norm (\sigma)) (\norm (\tau_1)) = (\widehat\norm (\sigma)) (\norm (\tau_2)) \]
			Par bonne formation de $\sigma$ et le lemme \ref{subst_bf_implique_norm_bf}, cet unificateur est bien formé.
		\item[$(\Leftarrow)$]
			Soient deux types $\tau_1$ et $\tau_2$ tels que leurs normalisées admettent un unificateur bien formé $\theta$ :
			\[ \hat\theta (\norm (\tau_1)) = \hat\theta (\norm (\tau_2)) \]
			Par bonne formation de $\theta$ et le lemme \ref{subst_bf_implique_subst_norm_eq_norm_inorm_subst}, il vient :
			\[ \norm ((\widehat\inorm (\theta))(\tau_1)) = \norm ((\widehat\inorm (\theta))(\tau_2)) \]
			Par le lemme \ref{equivT_equiv_egaliteN}, $\inorm (\theta)$ est un unificateur de $\tau_1$ et $\tau_2$ :
			\[ (\widehat\inorm (\theta)) (\tau_1) \equiv_\T (\widehat\inorm (\theta)) (\tau_2) \]
			Par bonne formation de $\theta$ et le lemme \ref{subst_bf_implique_inorm_bf}, cet unificateur est bien formé.
	\end{itemize}
\end{preuve}

\begin{lemme} \label{equivT_equiv_egaliteN}
...
\begin{lemme} \label{norm_subst_eq_norm_subst_norm}
	Pour tout $\sigma$ dans $\Sigma^{\bold{bf}}$, on a :
	\[ \norm \circ \hat\sigma = \widehat\norm (\sigma) \circ \norm \]
\end{lemme}
...
\begin{lemme} \label{bf_implique_norm_inorm_eq_id}
	La composée de $\inorm$ par $\norm$ est l'identité sur $\N^{\bold{bf}}$.
\end{lemme}
...
\begin{lemme} \label{subst_bf_implique_norm_inorm_eq_id}
	La composée de $\inorm$ par $\norm$ est l'identité sur $\Theta^{\bold{bf}}$.
\end{lemme}

\begin{preuve}
	Soient $\theta$ dans $\Theta^{\bold{bf}}$ et $\alpha$ dans $\V$. \\
	Par définition de $\norm$ puis $\inorm$, il vient :
	\[ (\norm (\inorm (\theta))) (\alpha) = \norm (\inorm (\theta (\alpha))) \]
	Par bonne formation de $\theta$ et le lemme \ref{bf_implique_norm_inorm_eq_id}, on obtient :
	\[ (\norm (\inorm (\theta))) (\alpha) = \theta (\alpha) \]
	C'est bien le résultat attendu.
\end{preuve}

\begin{lemme} \label{subst_bf_implique_subst_norm_eq_norm_inorm_subst}
	Pour tout $\theta$ dans $\Theta^{\bold{bf}}$, on a :
	\[ \hat\theta \circ \norm = \norm \circ \widehat\inorm (\theta) \]
\end{lemme}

\begin{preuve}
	Par le lemme \ref{subst_bf_implique_inorm_bf}, $\inorm (\theta)$ est bien formée car $\theta$ l'est. \\
	Par le lemme \ref{norm_subst_eq_norm_subst_norm}, on a donc :
	\[ \norm \circ \widehat\inorm (\theta) = \widehat\norm (\inorm (\theta)) \circ \norm \]
	Le lemme \ref{subst_bf_implique_norm_inorm_eq_id} nous donne le résultat attendu :
	\[ \norm \circ \widehat\inorm (\theta) = \hat\theta \circ \norm \]
\end{preuve}

\begin{lemme} \label{subst_bf_equiv_norm_subst_neq_norm_unit}
	Une substitution de types $\sigma$ est bien formée \ssi :
	\[ \forall \alpha,\ \norm (\sigma (\alpha)) \neq \mset{} \]
\end{lemme}

\begin{preuve}
	Les deux sens se montrent aisément par l'absurde en invoquant le lemme \ref{equivT_equiv_egaliteN}.
\end{preuve}

\begin{lemme} \label{subst_bf_implique_norm_bf}
	La normalisée de toute substitution de types bien formée est bien formée.
\end{lemme}

\begin{preuve}
	Soient $\sigma$ dans $\Sigma^{\bold{bf}}$ et $\alpha$ dans $\V$. \\
	Par définition de $\norm (\sigma)$, on a :
	\[ ( \norm (\sigma) ) (\alpha) = \norm (\sigma (\alpha)) \]
	Par le lemme \ref{norm_bf}, cette dernière quantité est bien formée. Par bonne formation de $\sigma$ et le lemme \ref{subst_bf_equiv_norm_subst_neq_norm_unit}, elle est par ailleurs différente du multi-ensemble vide. \\
	On en déduit la bonne formation de $\norm (\sigma)$.
\end{preuve}

\begin{lemme} \label{subst_bf_implique_inorm_bf}
	La normalisée inverse de toute substitution de types normalisés bien formée est bien formée.
\end{lemme}

\begin{preuve}
	Soient $\theta$ dans $\Theta^{\bold{bf}}$ et $\alpha$ dans $\V$. \\
	Par bonne formation de $\theta$ et le lemme \ref{subst_bf_implique_norm_inorm_eq_id}, on a :
	\[ \norm ((\inorm (\theta)) (\alpha)) = \norm (\inorm (\theta (\alpha))) = \theta (\alpha) \]
	Par bonne formation de $\theta$, cette dernière quantité est différente du multi-ensemble vide. \\
	Le lemme \ref{subst_bf_equiv_norm_subst_neq_norm_unit} nous donne la bonne formation de $\inorm (\theta)$.
\end{preuve}
...
\subsection{Première condition nécessaire}
...
\subsection{Deuxième condition nécessaire}
