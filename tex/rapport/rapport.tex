...
\reformule{\reformule{S'il est une chose dont on a besoin en programmation fonctionnelle}, c'est bien de fonctions. \reformule{Seulement voilà, ce n'est pas si simple.} Mettre la main sur l'une d'entre elles en présence d'un environnement de grande taille s'avère parfois difficile, à tout le moins fastidieux. \reformule{Aussi, remédions.}}
...
\reformule{Que faire, alors ? }Une recherche par spécification s'avérerait bien plus satisfaisante, mais pas forcément décidable. On peut retenir comme spécification grossière le type d'une fonction — « \textit{type as search key} » — tout en l'enrichissant par une approche sémantique et non plus purement syntaxique. La \textit{recherche modulo isomorphisme de type} s'est imposée dans la littérature. Elle consiste intuitivement à autoriser des réarrangements triviaux dans les types : permutations des paramètres et curryfication. Cette notion reste néanmoins à définir formellement, et surtout à décider algorithmiquement. Nous y consacrerons le chapitre 2.
...
\gab{Ici, il faut donner l'intention: pour la plupart des types, $encode$ est appelé durant la construction de la base, et $compat$ est appelé pendant la recherche dans la base. Le but est que $compat$ soit rapide.}
...
\gab{Il faut (re)donner encode/compat !}
...
Dans ce dernier chapitre, nous visitons les fonctionnalités du programme \dowsindex. On pourra en trouver le code source sur le dépôt \textit{github} \texttt{Drup/dowsing}.\gab{Un lien!} La version actuelle est compatible avec les systèmes d'exploitation Linux et macOS.
...
\gab{\og Par default, ca prend tout les paquest, donc ... \fg}
