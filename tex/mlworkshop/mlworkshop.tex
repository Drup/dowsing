...
Such an unification algorithm has already been proposed~\cite {boudet}, still its intrinsic complexity leads to actual search times that can reach up to xx seconds on the personal laptop of the most OCaml fan of the coauthors of this abstract. 
...
\end{compactitem}
Features are designed such that if the type request feature is different from a given candidate feature in the database, then we know for sure that they cannot be unified.  The challenge is then to design features that are both discriminating and compatible with  our notion of unification. 

\medskip
We have so far identified three such features, for instance:
