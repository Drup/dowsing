...
let rec of_outcometree_rec vars x : Raw.t =
  ...
  | Otyp_tuple tup ->
    let tup' =
      List.map (of_outcometree_rec vars) tup
    in
    Raw.tuple tup'
  | Otyp_var (_, s) ->
    Raw.var vars (Some s)
...
let rec of_parse_rec resolver vars x : Raw.t =
  let open Parsetree in match x.ptyp_desc with
  | Ptyp_arrow (_label, arg, ret) ->
    let arg' = of_parse_rec resolver vars arg in
    let ret' = of_parse_rec resolver vars ret in
    Raw.arrow arg' ret'
  | Ptyp_constr (id, args) ->
    let a =
      S.to_array @@
      S.map (of_parse_rec resolver vars) @@
      S.of_list args
    in
  ...
  | Ptyp_tuple tup ->
    let tup' =
      List.map (of_parse_rec resolver vars) tup
    in
    Raw.tuple tup'
  | Ptyp_any ->
    Raw.var vars None
  ...
  | Ptyp_poly (_, ty) -> of_parse_rec resolver vars ty
...
let of_parsetree ?ht ?(resolver=default_resolver) x =
  let vars = Raw.varset () in
  let t = of_parse_rec resolver vars x in
  normalize ?ht t
